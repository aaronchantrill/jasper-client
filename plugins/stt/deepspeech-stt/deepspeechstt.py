...
class DeepSpeechSTTPlugin(plugin.STTPlugin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Clear the audiolog
        files=os.listdir(os.path.join(os.path.dirname(os.path.realpath(__file__)),"audiolog"))
        for file in files:
            if file.endswith(".wav"):
                self._logger.info("to delete: %s"%os.path.join(os.path.dirname(os.path.realpath(__file__)),"audiolog",file))
                os.remove(os.path.join(os.path.dirname(os.path.realpath(__file__)),"audiolog",file))
        self._filecount=0
